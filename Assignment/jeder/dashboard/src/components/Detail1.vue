...
export default {
    ...
    methods: {
        ...
        updateChart() {
            ...
            let terrorismData_prepared = this.store.terrorismDataFiltered.map(d => {
                return {
                    ...d,
            ...
            content.selectAll("myPath")
                ...
                .join("path")
                .attr("d", d => d3.line()(dimensions.map((p) => [x(p) == NaN ? 0 : x(p), y[p]((d[p] == NaN ? 0 : d[p]) )])))
                .style("fill", "none")
                .style("stroke", d => {
...
<template>
    <h3 class="ma-2"> Events Details</h3>
    <svg id="det1">
    </svg>
</template>
