...
export default {
    ...
    methods: {
        ...
        initChart() {
            ...
            let colorScale = d3.scaleOrdinal().domain(clusters).range(d3.schemeTableau10)
            ...
            const highlight = function(d: Object) {
                ...
                d3.selectAll("." + class_name)
                    ...
                    .style("stroke", colorScale(class_name) as string)
                    .style("opacity", 1)
            }

            const normal = (d: Object) => {
                d3.selectAll(".point")
                    .data(this.points)
                    .transition().duration(200).delay(1000)
                    .style("stroke", (d) => { return colorScale(clusters[d.cluster]) as string})
                    .style("opacity", .5)
            }
            ...
            const points = chartContainer.append('g')
                ...
                .attr('data-class', (d: ScatterPoint) => this.clusters[d.cluster])
                .attr('class', (d: ScatterPoint) => 'point ' + this.clusters[d.cluster])
                .attr('r', 5)
            ...
            const title = chartContainer.append('g')
                ...
                .text('Dry Bean Dataset PCA Projection')
        },
        initLegend() {
...
<template>
    <div class="viz-container d-flex justify-end">
        ...
        <div id="scatter-legend-container" class="d-flex">
            <svg id="scatter-legend-svg" width="100%" height="100%"></svg>
        </div>
    </div>
</template>
