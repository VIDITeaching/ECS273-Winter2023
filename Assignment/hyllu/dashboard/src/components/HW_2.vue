...
export default {
    ...
    methods: {
        ...
        initChart() {
            let chartContainer = d3.select('#scatter-svg-2')
        ...
        initLegend() {
            let legendContainer = d3.select('#scatter-legend-svg-2')
    ...
    watch: { // updated because a legend is added.
        rerender(newSize) {
            if (!isEmpty(newSize)) {
                d3.select('#scatter-svg-2').selectAll('*').remove()
                d3.select('#scatter-legend-svg-2').selectAll('*').remove()
                this.initChart()
                this.initLegend()
            }
        },
...
<template>
    <div class="viz-container d-flex justify-end">
        <div class="chart-container d-flex" ref="scatterContainer">
            <svg id="scatter-svg-2" width="100%" height="100%">
            </svg>
        </div>
        <div id="scatter-legend-container" class="d-flex">
            <svg id="scatter-legend-svg-2" width="100%" height="100%">
            </svg>
        </div>
    </div>
</template>
